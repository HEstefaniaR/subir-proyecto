...
async function obtenerCursosPorEstudianteNoPeriodo(nombreEstudiante, periodoExcluido) {
    return new Promise((resolve, reject) => {
        ...
        connection.query(query, [nombreEstudiante, periodoExcluido], (error, results) => {
            if (error) {
...
async function traerCursoPorNombreYGrupo(nombreCurso, grupo) {
    const [result] = await connection.query('SELECT * FROM cursos WHERE nombreCurso = ? AND grupo = ?', [nombreCurso, grupo]);
    return result[0];
}
