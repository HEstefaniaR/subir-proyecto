...
router.get('/cursos/profesor/:correoProfesor/:periodo', async (req, res) => {
    const { correoProfesor, periodo } = req.params;
    try {
        const cursos = await cursosModel.obtenerCursosPorProfesorYPeriodo(correoProfesor, 2024-3);
        res.json(cursos);
    } catch (error) {
